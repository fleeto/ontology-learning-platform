...
**8 类业务对象 · 7 类关系 · 6 类操作 · 6 阶段端到端演示 · 24 条真实案例 · 3 个行业交互模拟**
...
`/cases` 案例库收录 2026 年 Palantir 本体相关案例（AIPCon 10/11 登台客户、官方博客披露的真实部署、Method Security 网络安全本体合作项目），每条标注来源与可信度等级。其中三个提供交互式模拟，沿用首页的图谱、粒子与路径追踪交互：

| 模拟 | 场景 | 可调参数 |
| --- | --- | --- |
| 航网运营恢复 | 天气事件沿 link types 传播到航班、机组与旅客，生成恢复方案 | 受影响枢纽、天气持续小时 |
| 软件工厂交付 | 需求经 Agent 起草、自动化测试、人工审批门到部署 | 需求复杂度、是否启用 Agent 辅助 |
| 文档知识图谱 | 非结构化文档到实体消歧、图谱构建与保留问题验证 | 文档批次规模、消歧严格度 |

模拟中的客户与场景为官方披露事实，具体对象、记录数与计算结果为浏览器内模拟，并在页面顶部按「官方事实 / 官方声称 / 模拟数据」三级逐条标注。案例库教材（`textbook/案例库.md`）由 `npm run gen:cases` 从同一份事实数据生成，`npm run verify:cases` 校验各案例在边界参数下的计算与阶段引用。
...
| 真实案例 | 案例库 | AIPCon 10/11 客户、官方博客案例与可信度说明 | `/cases` |
| 真实案例 | 航网运营恢复 | American Airlines：天气事件沿关系传播与恢复 | `/cases/american-airlines` |
| 真实案例 | 软件工厂交付 | 陆军软件工厂 + Agentik：Agent 起草、人工审批 | `/cases/army-software-factory` |
| 真实案例 | 文档知识图谱 | 4000 万文档结构化与保留问题验证 | `/cases/document-graph` |
...
```text
src/
├── app/                    # 首页、本体建模、数据集成、应用与案例页面
├── components/
│   ├── OntologyJourney.tsx  # 六阶段端到端教学演示
│   ├── CaseSimulator.tsx   # 案例模拟引擎（数据驱动，无领域逻辑）
│   └── Layout.tsx          # 导航与页面布局
├── data/
│   ├── ontology-model.ts   # 对象、关系、操作与接口定义
│   └── cases/              # 案例数据包与案例库事实源
└── lib/
    └── graph-geometry.ts   # 图谱几何、粒子与路径查找（首页与案例共用）
scripts/                    # 案例库生成与校验脚本
textbook/                   # 完整课程、案例与练习
materials/                  # 四节课程的研究笔记
refs/                       # 参考资料
...
[审查清单与改进说明](docs/ontology-demo-review.md)记录模型语义、审批状态、组装台与图谱交互的修复。运行 `npm run verify:simulation` 验证演示状态边界；工作流审批由 Action 模型驱动，另设到货确认。
